...
class UsersController {
  ...
  initializeRouter(): void {
    this.router.get("/redirect",passport.authenticate('google',{failureRedirect:"/",successRedirect:"/api/test"}))
    this.router.get("/register/google",passport.authenticate('google',{scope:["profile","email"]}))
    ...
    this.router.post("/login",validationMiddleware(usersDto.login),this.login)
  ...
  private googleAuth = async (req:Request,res:Response,next:NextFunction):Promise<Response|void>=>{
    try{
